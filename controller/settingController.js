...
const updateGlobalSetting = async (req, res) => {
	try {
		...
		const globalSetting = await Setting.findOneAndUpdate({ name: "globalSetting" }, { $set: setObject }, { new: true, upsert: true });
...
const getStoreCustomizationSetting = async (req, res) => {
	try {
		...
		const storeCustomizationSetting = await Setting.findOne({ name: "storeCustomizationSetting" }, projection);
